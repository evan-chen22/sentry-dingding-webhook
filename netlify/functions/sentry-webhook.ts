 ...
 */
export const handler = async (event: any, _context: any): Promise<WebhookResponse> => {
  // 设置CORS头
  const headers = {
  ...
  try {
    ...
    if (!dingdingWebhookUrl) {
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: 'DingDing webhook URL not configured' } as ErrorResponse)
      };
    }
...
function formatSentryMessage(sentryData: SentryWebhookPayload): string {
  ...
  if (browser.name || os.name || device.family) {
    markdown += `**设备信息**:\n`;
    if (browser.name) markdown += `- 浏览器: \`${browser.name} ${browser.version}\`\n`;
